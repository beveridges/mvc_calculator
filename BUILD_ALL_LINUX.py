...
# Ensure UTF-8 output (safer with emojis / arrows etc.)
sys.stdout.reconfigure(encoding="utf-8")
sys.stderr.reconfigure(encoding="utf-8")

ROOT = Path(__file__).resolve().parent

PORTABLE_SCRIPT = ROOT / "build_linux_portable.py"
APPIMAGE_SCRIPT = ROOT / "build_linux_appimage.py"
DEB_SCRIPT      = ROOT / "build_linux_deb.py"
...
def main() -> None:
    ...
    # 1) ALWAYS rebuild portable onedir (so Linux is in sync with Windows build)
    run_step("Portable Linux Build", [sys.executable, str(PORTABLE_SCRIPT)])

    # 2) AppImage (versioned using utilities/version_info.BUILDNUMBER)
    run_step("AppImage Build", [sys.executable, str(APPIMAGE_SCRIPT)])

    # 3) .deb package (same BUILDNUMBER)
...
# # -----------------------------------
# # Builds:
# #   ✔ Portable Linux onedir (ALWAYS rebuilt)
# #   ✔ AppImage
# #   ✔ .deb package
...
# print("\n✅ ALL LINUX ARTIFACTS BUILT SUCCESSFULLY (PORTABLE INCLUDED)\n")
