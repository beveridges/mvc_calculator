...
def main():
    ...
    VERSION_INFO_FILE = REPO_ROOT / "utilities" / "version_info.py"
    ...
    if BUILDNUMBER == "unknown":
        ...
        print(f"   File location: {REPO_ROOT / 'utilities' / 'version_info.py'}")
        print(f"   Make sure you've run BUILD_ALL_WINDOWS.py first!")
        sys.exit(1)
    
    # Clear Python cache files to force fresh imports in build scripts
    import glob
    cache_pattern = str(REPO_ROOT / "utilities" / "__pycache__" / "version_info*.pyc")
    ...
    run_step("Portable Linux Build", portable_cmd, TEMP_LOG_FILE)
    ...
    if args.oa:
        oa_portable_cmd = [sys.executable, str(PORTABLE_SCRIPT), "--oa"]
        if args.preactivated_entitlement:
            oa_portable_cmd += ["--preactivated-entitlement", args.preactivated_entitlement]
        run_step("OA Portable Linux Build", oa_portable_cmd, TEMP_LOG_FILE)
