...
def find_logo_path():
    ...
    resources_dir = REPO_ROOT / "resources" / "icons"
...
def main():
    ...
    try:
        ...
        lts = sorted_versions[2] if len(sorted_versions) > 2 else None

        print(f"Latest version: {latest}")
        if prev:
            print(f"Previous version: {prev}")
        if lts:
            print(f"LTS version: {lts}")
    ...
    <div class="release-notes-text">
        ...
        # Long Term Support (LTS) — third newest licensed build when at least three exist
        if lts and lts in versions:
            lts_notes = load_notes(lts)
            lts_date = lts_notes["date"] or datetime.now().strftime("%d %b %Y")
            lts_desc_raw = (lts_notes.get("description") or "").strip()
            lts_desc = lts_desc_raw or (
                f"A stable release of version {lts}, designed for long-term use, reliability, and production environments."
            )
            lts_whats_new_html = make_li_list(lts_notes["whats_new"]) if lts_notes["whats_new"] else "<li>No new features in this release.</li>"
            lts_bug_fixes_html = make_li_list(lts_notes["bug_fixes"]) if lts_notes["bug_fixes"] else "<li>No bug fixes in this release.</li>"
            lts_tags_html = " ".join(
                f"<span class='tag tag-{normalize_tag_name(t)}'>{format_tag_display(t)}</span>"
                for t in lts_notes["tags"]
                if t and t.strip()
            )
            if lts_tags_html:
                lts_tags_html = f"<div style='margin-top: 10px;'>{lts_tags_html}</div>"
            lts_section = f'''<hr class="divider">
...
<div class="release-section">
    ...
    <div>
        <div class="section-title">Long Term Support (LTS) Release — {lts}</div>
    ...
    <div class="release-notes-text">
        ...
        {build_table(versions[lts])}

    </div>
</div>'''
            html = html.replace("{{LTS_SECTION}}", lts_section)
        else:
            html = html.replace("{{LTS_SECTION}}", "")
        ...
        if oa_version and oa_files:
            ...
            oa_desc_raw = (oa_notes.get("description") or "").strip()
            oa_desc = oa_desc_raw or (
                f"A development build of version {oa_version}, intended for internal testing only and may be unstable."
            )
