...
# -*- coding: utf-8 -*-
"""
Fix Markdown rendering for MkDocs
---------------------------------
Converts docs/_index_raw.md → docs/index.md as pure Markdown
and restores admonition blocks correctly.
"""
...
# ------------------------------------------------------------
# 1. Convert using Pandoc → GitHub-Flavored Markdown
# ------------------------------------------------------------
...
print(f"✅ Markdown headings fixed and written to {out}")


# ------------------------------------------------------------
# 2. Restore custom admonitions
# ------------------------------------------------------------

def restore_admonitions(text: str) -> str:
    """
    Restores Markdown admonitions of the form:
        !!! type "Optional Title"
            body...
    Pandoc collapses these; this reconstructs them cleanly.
    """

    pattern = re.compile(
        r"!!!\s+([A-Za-z0-9_-]+)"           # admonition type
        r"(?:\s+\"([^\"]+)\")?"             # optional "title"
        r"\s*(.*?)(?=\n{2,}|\Z)",           # body until blank line
        re.DOTALL,
    )
    ...
    def convert(match: re.Match) -> str:
        ...
        if not title:
            ...
            if title_match:
                ...
                body = title_match.group(2).lstrip("\n")

        # indent body (4 spaces) the MkDocs way
        if body:
            body_lines = [
...
# ------------------------------------------------------------
# 3. Apply restore + save output
# ------------------------------------------------------------

try:
    rendered = out.read_text(encoding="utf-8")
    restored = restore_admonitions(rendered)

    if restored != rendered:
        out.write_text(restored, encoding="utf-8")
        print("[ok] Restored admonition formatting in docs/index.md")

except Exception as exc:
    print(f"[warn] Could not restore admonitions: {exc}")
...
# ---------------------------------
# Converts docs/_index_raw.md → docs/index.md as pure Markdown
# so that #, ##, etc. headings render properly.
# """
...
# pypandoc.convert_file(
    # str(raw),
    # to="gfm",  # GitHub-Flavored Markdown = fully compatible with MkDocs
    # outputfile=str(out),
    # extra_args=[
        # "--citeproc",
        # "--strip-comments",
...
# # • Ensures a clean “## References” heading before the reference list
# # • Outputs real Markdown (not HTML), so MkDocs TOC headings work
# # """
...
# # # Safety: remove accidental “site/_index_raw/” folder
# # bad_folder = Path("site/_index_raw")
# # if bad_folder.exists() and bad_folder.is_dir():
    # # print(f"⚠️ Removing stray folder {bad_folder}")
    # # shutil.rmtree(bad_folder)

# # # =============================================================================
# # # 2. Read + Convert Markdown → Markdown (NOT HTML)
# # # =============================================================================
# # markdown_source = RAW_MD.read_text(encoding="utf-8")

# # converted_md = pypandoc.convert_text(
    # # markdown_source,
    # # to="markdown_strict",  # ✅ keep Markdown syntax intact
    # # format="md",
    # # extra_args=[
        # # "--citeproc",
        # # "--strip-comments",
...
# # converted_md = re.sub(r"<!--.*?-->", "", converted_md, flags=re.DOTALL)

# # # Normalize “References” heading
# # converted_md = re.sub(
    # # r"(?:^|\n)(?:#+\s*)?references\s*(?:#+)?",
    # # "\n## References",
    # # converted_md,
...
# # converted_md = re.sub(r"\n{3,}", "\n\n", converted_md).strip()

# # # =============================================================================
# # # 4. Write file
# # # =============================================================================
# # OUT_MD.parent.mkdir(parents=True, exist_ok=True)
# # OUT_MD.write_text(converted_md + "\n", encoding="utf-8")
