...
1. **Verify File Format**:
   - Ensure files are valid MATLAB `.mat` format
   - Try opening the file in MATLAB to verify it's not corrupted
...
4. **File Structure**:
   ...
   - Files from Qualisys or Noraxon should work automatically
...
1. **Check Data Structure**:
   - The application looks for the first non-metadata variable in the `.mat` file
   - Ensure your data variable is properly named
...
**Solutions**:
1. **Adjust Parameters**:
   - **Minimum Silence Duration**: Increase if finding too many bursts
   - **Minimum Sound Duration**: Increase to filter out short spikes
...
2. **Check Plot Area**:
   - Ensure you're clicking on the signal plot area
...
4. **Clear Previous Selections**:
   - Use the clear button (broom icon) if needed
...
**Solutions**:
1. **Check Selections**:
   - Ensure at least one burst is selected for each sensor
   - Red indicators show which sensors need selections
   - Check the console for status messages

2. **Multiple Bursts**:
   - For "best of 3" protocol, select at least 3 bursts
...
4. **Log Files**: Use "Send Log" button or attach log files from:
   - `%APPDATA%\MVC_Calculator\logs\`

### Sending Logs

1. Click the **Send Log** button in the console area
2. Or manually email logs from: `%APPDATA%\MVC_Calculator\logs\`
...
1. Open in MATLAB and check variable names
