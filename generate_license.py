...
from utilities.license import generate_license_key, get_machine_id
...
def _build_output_filename() -> Path:
    ...
    return Path(f"license-{timestamp}-{CLIENT_CODE}.key")
...
def main():
    ...
    license_file = _build_output_filename()
    ...
    print("  Filename format: license-YYYYMMDDTHHMMSSZ-CLT0002.key")
    print("\nLicense details:")
    print(f"  Email: {email}")
    print(f"  Country: {country}")
    ...
    print("  Payload format: unchanged (base64(email|country|hwid|expiration|signature))")
    print()
