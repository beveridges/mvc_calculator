...
# --- State ---
active_row = 0                        # <- HARDCODED to the first row
selections = {i: [] for i in range(nrows)}
patches = {i: [] for i in range(nrows)}
epsilon = 2.0                          # tolerance for deletion

def onselect(xmin, xmax):
    """Left-drag selection on the active row only."""
    if xmin == xmax:
        return
    if len(selections[active_row]) >= 3:
        print(f"Row {active_row}: already has 3 selections, ignoring.")
        return

    lo, hi = sorted([xmin, xmax])
    selections[active_row].append((lo, hi))
    patch = axes[active_row].axvspan(lo, hi, color="orange", alpha=0.3)
    patches[active_row].append(patch)

    print(f"Row {active_row}: selections now {selections[active_row]}")
    axes[active_row].set_xlim(xlim)
    fig.canvas.draw_idle()

def on_mouse_press(event):
    """Shift + Right-click deletes a selection on the active row."""
    if event.inaxes != axes[active_row]:
        return
    ...
    x_click = event.xdata
    idx = None
    for j, (lo, hi) in enumerate(selections[active_row]):
        if lo - epsilon <= x_click <= hi + epsilon:
            idx = j
            break

    if idx is not None:
        patches[active_row][idx].remove()
        patches[active_row].pop(idx)
        removed = selections[active_row].pop(idx)
        print(f"Row {active_row}: removed {removed}; remaining {selections[active_row]}")
        fig.canvas.draw_idle()
...
# Create a single SpanSelector only on the active row (first subplot)
span = SpanSelector(
    axes[active_row],
    onselect,
